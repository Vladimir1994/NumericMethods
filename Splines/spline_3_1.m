function [x_result, y_result] = spline_3_1(x, y, start_d)
    ...
    coeffs = linsolve(A, b);
    x_result = x(1):.01:x(3);
    y_result = coeffs(1) .* x_result .^ 3 + coeffs(2) .* ...
        x_result .^ 2 + coeffs(3) .* x_result + coeffs(4);
    for i = 3:2:fix(length(x) / 2) * 2 - 3 
        xx = x(i):.01:x(i + 2);
        yy = coeffs(2 * i - 1) .* xx .^ 3 + coeffs(2 * i) .* xx .^ 2 ...
            + coeffs(2 * i + 1) .* xx + coeffs(2 * i + 2);
        x_result = [x_result xx];
        y_result = [y_result yy];
    end
    if(m == 1)
        [xx, yy] = spline_2_1([x(length(x) - 1) x(length(x))], ...
        ...
        yy = coeffs(length(coeffs) - 3) .* xx .^ 3 + ...
            coeffs(length(coeffs) - 2) .* xx .^ 2 + ...
            coeffs(length(coeffs) - 1) .* xx + coeffs(length(coeffs));
        x_result = [x_result xx];
        y_result = [y_result yy];
    end
end